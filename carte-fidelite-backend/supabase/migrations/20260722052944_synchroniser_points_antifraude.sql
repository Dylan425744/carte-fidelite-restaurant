-- Aligne la protection anti-fraude sur le nombre de points reellement attribue
-- par un passage. Une limite anti-fraude ne peut pas invalider le scan normal
...
select
  r.id,
  greatest(10, coalesce(r.points_per_scan, 10)),
  greatest(50, coalesce(r.points_per_scan, 10))
from public.restaurants r
on conflict (restaurant_id) do update
set max_points_per_scan = greatest(
      public.fraud_settings.max_points_per_scan,
      excluded.max_points_per_scan
    ),
    max_points_per_day = greatest(
      public.fraud_settings.max_points_per_day,
      excluded.max_points_per_day
    ),
    updated_at = now();
